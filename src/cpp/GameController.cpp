...
void GameController::movePacman() {
    ...
    if (nextX < 0) nextX = Graph::WIDTH - 1;
    if (nextX >= Graph::WIDTH) nextX = 0;
    if (nextY < 0) nextY = Graph::HEIGHT - 1;
    if (nextY >= Graph::HEIGHT) nextY = 0;
...
Pacman* GameController::getPacman() const {
    return pacman;
}
...
double GameController::getSurvivedTime() const {
    return survivedTime;
}
