 ...
 * neighbor that minimizes the distance to this predicted location.
 *
...
Location AggressiveGreedyStrategy::findNextMove(Graph *graph, Entity *monster,
  ...
  if (targetDir.x != 0 || targetDir.y != 0) {
    ...
    int predX =
        ((targetLoc.x + targetDir.x * 2) % Graph::WIDTH + Graph::WIDTH) %
        Graph::WIDTH;
    int predY =
        ((targetLoc.y + targetDir.y * 2) % Graph::HEIGHT + Graph::HEIGHT) %
        Graph::HEIGHT;
